...
1. **不保留兼容层**。EditSpec v1 不迁移、不适配、不双写。现有 6 个项目的 EditSpec 视为历史产物，
   保留在 `projects/_archive/` 供参考，不参与新链路。
...
5. **数据资产保留，代码不保留**。`library/engine.sqlite` 中的素材、镜头、嵌入、
   审片决策与发布指标是真实积累，通过一次性 ETL 迁入新 schema；
   `anime/` 下的实现代码按新分层重写。
