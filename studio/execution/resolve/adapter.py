...
class ResolveAdapter:
    ...
    def build_demo_replica_comp(
        ...
        *,
        ...
        duration_frames: int,
        speed_ramp: bool,
        terminal_impact_frame: int | None,
    ):
        """Build the owner's strict demo-replica Fusion template."""
        if duration_frames < 6:
            raise ValueError("demo replica clip 至少需要 6 帧")
        comp, media_in, media_out = self._fresh_transform_comp(item)
        previous = media_in
        if speed_ramp:
            speed = comp.AddTool("TimeStretcher")
            speed.SetAttrs({"TOOLS_Name": "SpeedRamp_50_200_50"})
            speed.ConnectInput("Input", previous)
            self._connect_scalar_spline(
                comp,
                speed.SourceTime,
                "DemoSourceTime",
                self._beat_pull_curves(
                    duration_frames=duration_frames,
                    stage="accelerate",
                    intensity=1.0,
                    window_frames=min(8, duration_frames - 1),
                )["source_time"],
            )
            previous = speed
        base = comp.AddTool("Transform")
        beat = comp.AddTool("Transform")
        blur = comp.AddTool("DirectionalBlur")
        color = comp.AddTool("BrightnessContrast")
        if any(tool is None for tool in (base, beat, blur, color)):
            raise ResolveOperationError("demo replica Fusion 节点创建失败")
        base.SetAttrs({"TOOLS_Name": "Transform_Base"})
        beat.SetAttrs({"TOOLS_Name": "Transform_Beat"})
        blur.SetAttrs({"TOOLS_Name": "DirectionalBlur"})
        color.SetAttrs({"TOOLS_Name": "ColorCorrector"})
        base.ConnectInput("Input", previous)
        beat.ConnectInput("Input", base)
        blur.ConnectInput("Input", beat)
        color.ConnectInput("Input", blur)
        media_out.ConnectInput("Input", color)

        last = duration_frames - 1
        f2, f5, f17, f20 = min(2, last), min(5, last), min(17, last), min(20, last)
        sign = -1.0 if category in {"profile", "angry"} else 1.0
        size_curve = {0.0: 1.20, float(f2): 1.14, float(f5): 1.07,
                      float(f17): 1.09, float(f20): 1.10, float(last): 1.10}
        angle_curve = {0.0: sign * 0.6, float(f2): sign * 0.25,
                       float(f5): 0.0, float(last): 0.0}
        blur_curve = {0.0: 0.05, float(f2): 0.0175,
                      float(f5): 0.0, float(last): 0.0}
        if terminal_impact_frame is not None:
            terminal = float(min(last, max(0, terminal_impact_frame)))
            size_curve[terminal] = 1.20
            angle_curve[terminal] = sign * 0.6
            blur_curve[terminal] = 0.05
        self._connect_scalar_spline(
            comp, beat.Size, "DemoBeatSize",
            size_curve,
        )
        self._create_scalar_spline(
            comp, "DemoCenterX",
            {0.0: 0.5 + sign * 0.03, float(f2): 0.5 + sign * 0.015,
             float(f5): 0.5, float(f17): 0.5 + sign * 0.003,
             float(f20): 0.5 + sign * 0.004, float(last): 0.5 + sign * 0.004},
        )
        beat.Center.SetExpression("Point(DemoCenterX.Value,0.5)")
        self._connect_scalar_spline(
            comp, beat.Angle, "DemoBeatAngle",
            angle_curve,
        )
        self._connect_scalar_spline(
            comp, blur.Length, "DemoBeatBlur",
            blur_curve,
        )
        blur.SetInput("Angle", 90.0 if category == "standing" else 0.0)
        color.SetInput("Contrast", 0.05)
        color.SetInput("Saturation", 1.08)
        if terminal_impact_frame is not None:
            terminal = float(min(last, max(0, terminal_impact_frame)))
            self._connect_scalar_spline(
                comp,
                ...
                {
                    0.0: 1.0,
                    float(max(0, int(terminal) - 1)): 1.0,
                    terminal: 1.35,
                },
            )
        current = item.GetFusionCompNameList() or []
        if current and current[-1] != comp_name:
            item.RenameFusionCompByName(current[-1], comp_name)
        return comp
